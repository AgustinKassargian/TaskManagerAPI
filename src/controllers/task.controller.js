...
export const editTask = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Tarea actualizada correctamente.", data: updatedTask });
  } catch (error) {
    res.status(500).json({
  ...
  try {
    ...
    if (result.deletedCount === 0) {
      return res
        .status(404)
        .json({
          message: "No se encontraron tareas completadas para eliminar.",
        });
    }
    ...
    res
      .status(200)
      .json({
        message: `${result.deletedCount} tarea(s) eliminada(s) correctamente.`,
      });
  } catch (error) {
    console.error(error);
    res.status(500).json({
